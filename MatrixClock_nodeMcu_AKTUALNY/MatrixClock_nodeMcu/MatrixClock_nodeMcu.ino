...
#define bright  2      // button A0

char ssid[] = "DOM24_SALON";         				// your network SSID (name)
char pass[] = "38843884";     				// your network password
...
//    { 0x07, 0x0e, 0x11, 0x11, 0x0f, 0x01, 0x02, 0x0c, 0x00 },   // 0x39, 9
    ...
    { 0x07, 0x0e, 0x1b, 0x1b, 0x1b, 0x1b, 0x1b, 0x0e, 0x00 },   // 0x30, 0
    { 0x07, 0x6, 0xe, 0x6, 0x6, 0x6, 0x6, 0x6, 0x0 },   // 0x31, 1
    { 0x07, 0xe, 0x1b, 0x3, 0x6, 0xc, 0x19, 0x1f, 0x0 },   // 0x32, 2    
		{ 0x07, 0xe, 0x13, 0x3, 0x6, 0x3, 0x13, 0xe, 0x0 },   // 0x33, 3
		{ 0x07, 0x7, 0xb, 0x1b, 0x13, 0x1f, 0x3, 0x3, 0x0 },   // 0x34, 4
		{ 0x07, 0x1f, 0x18, 0x1e, 0x1b, 0x3, 0x1b, 0xe, 0x0 },   // 0x35, 5
		{ 0x07, 0xe, 0x19, 0x18, 0x1e, 0x1b, 0x1b, 0xe, 0x0 },   // 0x36, 6
		{ 0x07, 0x1f, 0x13, 0x3, 0x6, 0xc, 0xc, 0xc, 0x0 },   // 0x37, 7
		{ 0x07, 0xe, 0x1b, 0x1b, 0xe, 0x1b, 0x1b, 0xe, 0x0 },   // 0x38, 8
		{ 0x07, 0xe, 0x1b, 0x1b, 0xf, 0x3, 0x13, 0xe, 0x0 },   // 0x39, 9
		{ 0x04, 0x00, 0x00, 0x03, 0x03, 0x00, 0x03, 0x03, 0x00 },   // 0x3a, :
...
tm* connectNTP() { //if response from NTP was succesfull return *tm else return a nullpointer
  ...
  Serial.println("highWord: " + String(highWord));
  Serial.println("lowhWord: " + String(lowWord));
...
Stunde = Stunde + ZDiff;
 //Stunde = Stunde + 1;              // BallD: winter time
...
void timer50ms() {
	...
	f_tckr50ms = true;
	cnt50ms++;
  if (cnt50ms == 10) { //500ms to divider
    f_tck500ms = true;
  }
	if (cnt50ms == 20) {
		f_tckr1s = true; // 1 sec
...
void loop() {
  max7219_set_brightness(0);
	...
	while (true) {
		yield();
		if (f_tckr1h == true) { //syncronize RTC every hour
			f_tckr1h = false;
			tm* tt;
			tt = connectNTP();
			if (tt != NULL)
		...
		if (f_tckr1s == true)        // flag 1sek
		{
			...
			std22 = std2;
			f_tckr1s = false;
			if (bright == HIGH) //if button pressed then scroll
				f_scroll_x = true;
		} // end 1s
		if (f_tckr50ms == true) {
			f_tckr50ms = false;

			if (f_scroll_x == true) { //scroll
				z_PosX++;
				d_PosX++;
				if (d_PosX == 95)
					z_PosX = 0;
